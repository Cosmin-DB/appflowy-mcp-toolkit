...
| Document/page Markdown editing | append Markdown supported (paragraphs, headings, lists, blockquotes); fetch/replace/block-level editing is backlog | route/unit coverage; browser-tested visible appended content; human-verified on AppFlowy Cloud 2026-05-18 |
| Database schema/rows/tasks | high | high for task lifecycle data plane | unit-tested; self-hosted; browser-tested Grid/Board lifecycle; human-verified visible AppFlowy Cloud task card on 2026-05-18 |
...
| Files/blobs | metadata plus v1 upload/download/delete | high for single-file flow | unit-tested; Docker/self-hosted and AppFlowy Cloud single-file upload/download/delete |
| Trash/favorites/recent | read implemented | high | unit-tested; selected page lifecycle smoke; AppFlowy Cloud restore/trash/delete-trash verified on test pages |
| Sharing/publishing | read-only publish metadata + gated publish/unpublish writes implemented | medium; safety-sensitive writes remain gated | unit-tested; browser-tested published public URL visibility; publish/unpublish human-verified on AppFlowy Cloud 2026-05-18 |
| Search | read implemented | medium; depends on search service | unit-tested |
| Templates | read-only discovery implemented; published-page instantiation implemented | medium; depends on template-center data in deployment | unit-tested against pinned AppFlowy template-center and published-duplicate routes; browser-tested published-page instantiation; duplicate published page human-verified on AppFlowy Cloud 2026-05-18 |
| Quick notes | CRUD implemented, dry-run writes | medium; live create/update/delete route responds, but list behavior needs more investigation | unit-tested; AppFlowy Cloud create/update/delete smoke, no browser/human validation |
...
| Database rows | list ids/details implemented | create/upsert implemented | upsert/update, status move, and row-id collab update implemented | no REST delete; Yjs row-order delete implemented | high | unit-tested; self-hosted; browser-tested task lifecycle; human-verified visible AppFlowy Cloud task card on 2026-05-18 | Broaden field type tests and keep collab mutations narrowly scoped |
...
| File storage/blobs | usage/list/metadata and v1 blob download implemented | v1 single-upload implemented; multipart routes mapped but deferred | multipart complete routes mapped but deferred | v1 blob delete implemented | v1 upload/download/delete Docker-proven and AppFlowy Cloud-proven; Media-cell linking Docker-proven | unit-tested; Docker/self-hosted upload/download/delete, AppFlowy Cloud upload/download/delete, and Media-cell link | Add multipart only if large-file demand appears |
| Trash | trash list implemented | n/a | single restore implemented dry-run default | single delete-from-trash implemented dry-run default | partial | unit-tested; single-page path covered by self-hosted lifecycle smoke; AppFlowy Cloud restore/trash/delete-trash smoke | Bulk restore/delete deferred |
...
| Publishing | namespace/default/list/get metadata + publish/unpublish writes implemented | publish routes confirmed | n/a | n/a | gated writes implemented; publish/unpublish with explicit double gate | unit-tested; browser-tested published public URL visibility; publish/unpublish human-verified on AppFlowy Cloud 2026-05-18 | Keep double-gated until broader sharing edge cases are tested |
| Search | search implemented | n/a | n/a | n/a | read implemented | unit-tested; service-dependent | AI summary endpoint deferred |
| Templates | category/creator/template/homepage routes + published-duplicate instantiation implemented | n/a | n/a | n/a | read-only discovery + published template instantiation implemented; arbitrary unpublished instantiation not supported | unit-tested against AppFlowy template-center and published-duplicate routes; browser-tested published-page instantiation; duplicate published page human-verified on AppFlowy Cloud 2026-05-18 | Add self-hosted smoke if local stack has seeded template-center data |
| Quick notes | list implemented | create implemented dry-run default | update implemented dry-run default | delete implemented dry-run default | CRUD implemented, but live list parity is uncertain | unit-tested; AppFlowy Cloud create/update/delete returned success, but list stayed empty | Investigate Quick Notes product behavior before claiming end-user readiness |
...
3. **Page/view management**
   ...
   - Keep document-body/Markdown editing separate from page organization. Friendly
     page Markdown fetch/replace tools are backlog until they have Yjs fixtures,
     self-hosted proof, and browser proof. Append Markdown now has browser proof.
